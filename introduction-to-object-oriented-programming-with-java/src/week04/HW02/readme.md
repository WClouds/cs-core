...
For the alphabetize operation, you will be using a pre-defined String method (compareTo) that compares two strings lexicographically, and returns an integer depending on which String is larger. If a 0 is returned, both Strings are lexicographically equal. A positive integer is returned if the first string is lexicographically greater than the second string, or else the result would be negative. See the example output for how the result should be printed to the user for the cases where Strings are equal.
...
Example Outputs
User input is bolded. Please make sure to follow the exact formatting as shown below. Note: Your program output should end with a new line. That is, after the last character of the answer or the error message, there should be a \n (you could use println, or print a \n manually - just make sure after the last line with text there is exactly one \n).
